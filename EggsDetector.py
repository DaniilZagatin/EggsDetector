...
class EggCounterApp:
    def __init__(self, root):
        ...
        # --- Пороговые значения ---
        # Белые (нижний порог) -> верхний порог [255,255,255]
        self.white_lower_r = StringVar(value="205")
        self.white_lower_g = StringVar(value="215")
        self.white_lower_b = StringVar(value="212")

        # Красные (нижний / верхний порог)
        self.red_lower_r = StringVar(value="90")
        self.red_lower_g = StringVar(value="45")
        self.red_lower_b = StringVar(value="45")

        self.red_upper_r = StringVar(value="205")
        self.red_upper_g = StringVar(value="160")
        self.red_upper_b = StringVar(value="140")
    ...
    def create_widgets(self):
        ...
        # --- Верхний фрейм (элементы управления) ---
        ...
        # --- Пороговые значения для красных ---
        red_thresh_frame = Frame(control_frame)
        red_thresh_frame.pack(side=LEFT, padx=10)

        Label(red_thresh_frame, text="Порог красных (min..max)",
              font=('Arial', 10, 'bold')).grid(row=0, column=0, columnspan=4)

        Label(red_thresh_frame, text="Min R:").grid(row=1, column=0, sticky=E)
        Entry(red_thresh_frame, textvariable=self.red_lower_r, width=5).grid(row=1, column=1)

        Label(red_thresh_frame, text="Min G:").grid(row=2, column=0, sticky=E)
        Entry(red_thresh_frame, textvariable=self.red_lower_g, width=5).grid(row=2, column=1)
        ...
        Label(red_thresh_frame, text="Max R:").grid(row=1, column=2, sticky=E)
        Entry(red_thresh_frame, textvariable=self.red_upper_r, width=5).grid(row=1, column=3)

        Label(red_thresh_frame, text="Max G:").grid(row=2, column=2, sticky=E)
        Entry(red_thresh_frame, textvariable=self.red_upper_g, width=5).grid(row=2, column=3)
        ...
        # --- Параметры для локальных максимумов (изолиний) ---
        ...
        # --- «3 окна» внизу (слева – оригинал, центр – обработанное, справа – лок. макс.) ---
    ...
    def count_eggs(self):
        ...
        # Красные
        r_lower = np.array([
            int(self.red_lower_r.get()),
            int(self.red_lower_g.get()),
            int(self.red_lower_b.get())
        ])
        r_upper = np.array([
            int(self.red_upper_r.get()),
            int(self.red_upper_g.get()),
            int(self.red_upper_b.get())
        ])

        mask_white = cv2.inRange(self.processed_image, w_lower, w_upper)
        mask_red = cv2.inRange(self.processed_image, r_lower, r_upper)
        ...
        # Красные
        mask_red = cv2.morphologyEx(mask_red, cv2.MORPH_OPEN, kernel, iterations=self.open_iter.get())
        mask_red = cv2.morphologyEx(mask_red, cv2.MORPH_CLOSE, kernel, iterations=self.close_iter.get())

        cont_white, _ = cv2.findContours(mask_white, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cont_red, _ = cv2.findContours(mask_red, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def save_results(self):
        ...
        if self.eggs_mask is None or self.processed_image is None or self.local_maxima_img is None:
            messagebox.showinfo("Сохранение", "Сначала загрузите изображение, выполните предобработку, "
    ...
    def validate_thresholds(self) -> bool:
        ...
        if not self.check_val(self.red_lower_r.get()): return False
        if not self.check_val(self.red_lower_g.get()): return False
        if not self.check_val(self.red_lower_b.get()): return False
        if not self.check_val(self.red_upper_r.get()): return False
        if not self.check_val(self.red_upper_g.get()): return False
